...
**Dashboard status (Recovery decision cards v1 — IMPLEMENTED).** The decision-card *content* + *choices* are now built on the dashboard from data the kernel already records (`docs/relix-dashboard-design.md` §6.10). The deterministic recovery model (`apps/dashboard/src/recovery.ts`) classifies a failed run by its structured `failure_class` (the Stage-1 buckets the kernel already computes) and a blocked task by its reopen eligibility + latest failed run, producing a plain-language root cause + recommendation + one-click choices (Retry / Resume / Reopen / Reopen & run / Reassign / Configure / Inspect / **Investigate**). The `RecoveryCard` renders this in the Work Run Detail (failed run) and Task Detail (blocked task) panels; every choice reuses an **existing** route (retry / resume / reopen[-and-run] / assign) or page (Crew / Settings) — **no new authority, no auto-reassign, no bypass** — and where no safe action exists it states what information is missing. **Investigate → chat companion is now wired (v1):** every card ends with an **Investigate with Prime** choice that seeds Prime with a safe, bounded, **redacted** investigation prompt (the task/run identity, the structured `failure_class` + failure text, the deterministic root cause + recommendation, and — on the run panel — the most-recent lines of the already-held redacted run-log tail), framed as a debugging question that **explicitly forbids creating tasks / starting runs / changing state / running tools**. The seed is handed off through a one-shot `sessionStorage` entry the Prime page consumes exactly once on mount and sends as the first message, so Prime answers it like a Hermes-style debugging partner (`apps/dashboard/src/investigateseed.ts`; `docs/relix-dashboard-design.md` §6.10) — materializing nothing.

**Diagnostic narrative pass (the cheap diagnostic LLM pass — IMPLEMENTED).** The Stage-2b "spawn a cheap diagnostic pass … *forbidden* from doing deliverable work — diagnosis only" is now built as an explicit, operator-triggered, **read-only** action. Every recovery card carries an **Analyze failure** choice (the lighter sibling of *Investigate with Prime* — a one-shot written diagnosis rather than a full chat); it appears on a failed-run card always, and on a blocked-task card only when the task's **latest run actually failed** (otherwise there is nothing for the model to read, so the card stays honest and offers no Analyze). Clicking it calls **`POST /v1/relux/runs/:id/diagnose`** (`crates/relux-kernel/src/server.rs::diagnose_run`), which under one short read-only lock fetches the run + its bounded, server-redacted log tail, builds a **bounded + redacted context** (`crates/relux-kernel/src/run_diagnosis.rs::DiagnosticContext` — the run id/status/`failure_class`/adapter, the task id/title, the redacted+clamped failure text, and the most-recent redacted log lines, each field clamped: failure 800 chars, log tail 2000 chars / 40 lines, title 160 chars), and — **off-lock** — hands it to the configured brain (`crate::ai::diagnose_via_openrouter`) under a no-authority system prompt that forbids claiming any action. The brain returns a concise **four-part narrative** (likely cause / evidence from the status·class·failure-text·logs / recommended next action / uncertainty + what to inspect next), re-redacted and clamped to 1500 chars. It **mutates nothing** — no tools, no task creation, no run change — and is gated to runs that actually recorded a failure (a healthy run gets an honest "nothing to diagnose", not a wasted call). **If no brain is configured** (or the provider hiccups) the route returns a clean **`mode: "unavailable"`** fallback that points the operator at the deterministic recovery card and offers the configure path — it never fabricates a diagnosis. The result renders **inline** on the card, **below** the still-visible deterministic card (`RecoveryCard`'s `diagnostic` block in `apps/dashboard/src/pages/Work.tsx`).

**Cross-Guild Inbox (the §5 attention queue — IMPLEMENTED).** The last remaining piece is now built: a true cross-Guild **Inbox** that collects the escalation items across the whole Guild into one prioritized list, rather than leaving them scattered across the Work detail panels and the oversight *Needs attention* strip. It is a **read-only projection** (`GET /v1/relux/inbox`, `crates/relux-kernel/src/server.rs::get_inbox`) composing four kinds — pending approvals, hard-failed runs, blocked tasks, and a paused Prime continuation — into unified items carrying a stable id, severity, plain title/summary, the related ids, and the recommended action kinds (each backed by an existing route). It honors this section's **"only the escalation kind reaches the Inbox"** contract directly: a failed run that is **silently auto-retrying** (a transient class with a non-exhausted retry budget) is **excluded**, so the queue stays signal, not noise. The dashboard renders it as the **Inbox** page (`apps/dashboard/src/pages/Inbox.tsx`) with a sidebar badge; every action reuses an existing route and nothing runs without the operator's click (`docs/relix-dashboard-design.md` §6.11). **Remaining future work:** triage SLAs / item ageing, cross-item grouping, and cross-project search.
