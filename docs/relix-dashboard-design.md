...
**Still pending (deliberately):** a true **Inbox** queue (§5) that collects escalation cards across the Guild — today the cards live on the Work detail panels next to the work, and the oversight strip's *Needs attention* list (§6.1) is the closest Inbox surface. The **Investigate → chat companion seeded with the diagnosis** choice is built (see *Investigate with Prime* above): a future enhancement is having Prime auto-pull the run transcript/logs into that first turn rather than relying on its own read-only tools when it needs more depth.
