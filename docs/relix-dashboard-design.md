...
**Still pending (deliberately):** per-subtree **cost** rollup on the board (the Relux run-cost ledger exists but is not yet joined here); drag-to-reorder the plan. These remain §6/§7 targets. (Ad-hoc (non-orchestration) task subtrees are now built — see §6.3.)
...
**Still pending (deliberately):** drag-to-reorder / reparent a subtask in the UI (the kernel cycle guard already covers reparenting safety); per-subtree **cost** rollup; auto-rolling child progress onto the parent's status. These remain §6/§7 targets.
...
**Still pending (deliberately):** free-form **drag-and-drop** (drag a card to a column) and keyboard drag/reorder; **reparent** / reorder a subtask; and a board move to a *non*-settable lane (re-open a blocked task to running) — re-opening is a run-lifecycle action, not a status decree, so it stays out of this slice. These remain §6/§7 targets.
