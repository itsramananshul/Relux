...
**Still pending (deliberately):** free-form **drag-and-drop** (drag a card to a column) and keyboard drag/reorder; **reorder** a subtask within a subtree; and a board move to a *non*-settable lane (re-open a blocked task to running) — re-opening is a run-lifecycle action, not a status decree, so it stays out of this slice. (Safe **reparent** of a subtask is now built as a selection control — see §6.6.) These remain §6/§7 targets.
...
**Still pending (deliberately):** free-form **drag-and-drop** reparent/reorder and keyboard drag; **reordering** siblings within a subtree (the `parent_task` edge has no sibling order today — children render in stable id order); and auto-rolling child progress onto the parent's status. These remain §6/§7 targets.
