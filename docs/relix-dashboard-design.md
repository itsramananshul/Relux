...
**Local-Prime "can't do this" recovery (the "running but nothing happens" fix).** A run on Prime's **local** adapter that refused a free-form external request — clone a repo, touch the filesystem/network, import a plugin — now fails closed (`Failed`, classified `adapter_missing`) and parks its task `Blocked`, instead of dangling in `Running` forever or fake-completing (see `docs/RELUX_MASTER_PLAN.md` §8.1 "Local Prime is deterministic"). `recovery.ts`'s `adapter_missing` branch detects this structurally — the run's `adapter_plugin` is `relux-adapter-local-prime` (local Prime is never a *missing* CLI) — and renders a tailored card: a plain-language root cause ("deterministic; performs no external work") and a recommendation pointing at the real remedies, with **Open Plugins** (the import flow, → `/plugins`) as the primary action plus **Reassign** (to a configured Claude/Codex adapter) and **Inspect** — never the generic "install your CLI" card. The new `open_plugins` action kind is wired in `Work.tsx` to the Plugins page; pinned by `apps/dashboard/test/recovery.test.ts` ("adapter_missing on the local Prime adapter points at plugin import + a real adapter").
...
- **Triage SLAs / ageing + filtering (IMPLEMENTED).** Closing the first of this section's "still future" items. Every item now carries an honest **age** so older urgent items are obvious. The crucial honesty point: the kernel runs on a **deterministic logical clock** (`crates/relux-kernel/src/clock.rs`), *not* wall-clock time — so the projection never claims a real-time duration or a fabricated deadline. The backend stamps each item with `attention_since` (the per-kind logical-clock anchor — an approval's `created_at`, a run's `ended_at`/`started_at`, a blocked task's `updated_at`, a paused continuation's `created_at_secs`) and `age_ticks` (logical-clock ticks — i.e. **kernel events** — elapsed since that anchor, measured against `kernel.clock_secs()`). The continuation carries an integer logical second directly (exact age); the string-stamped kinds parse the rendered stamp back to its seconds-of-day and take the wrap-adjusted distance (exact within a 24-logical-hour window). When a kind has **no usable anchor**, both fields are omitted and the row says **"age unavailable"** rather than inventing one (the *no fake deadlines* rule). The pure frontend `inbox.ts` bands `age_ticks` into a static, configurable **urgency bucket** — *fresh* (< 30 ticks) · *waiting* (< 120) · *stale* (< 600) · *overdue* (≥ 600) — rendered as a compact, restrained B&W **age badge** (only *stale*/*overdue* take color; the tick count reads "N ticks", never "N seconds"). The queue's order is now **severity first, then oldest-first within a severity** (a known age outranks an unknown one; because the bucket is monotonic in age, ordering by age descending already places *overdue* before *stale* before *waiting* before *fresh* — no separate bucket key needed). The page gains a **filter bar** — *All / Approvals / Failed runs / Blocked / Paused* plus a cheap **Overdue** band cut — each chip carrying its live count; a filtered-to-empty view shows a **filter-specific** empty line ("Nothing is overdue.") distinct from the global all-clear.
- **Cross-item grouping — collapsing a stalled subtree (IMPLEMENTED).** Closing this section's last "still future" item. A fanned-out failure (a parent effort whose children failed / blocked) no longer reads as a scatter of unrelated rows: related items now **collapse into one stalled-subtree card**. Grouping is built from the **one real relationship** the kernel exposes — the ad-hoc **`parent_task`** edge (`relux_core::Task`, populated via `create_task_with_parent` / the board reparent route). The backend projection now stamps each task-backed item (`failed_run` / `blocked_task`) with its **`parent_task`** (the underlying task's parent, when it has one) and the backend-resolved **`parent_title`** (so a card can be titled without a second fetch); approvals and the paused continuation carry no task edge and so are never grouped. The pure frontend `inbox.ts` (`buildInboxGroups`, `inboxGroupKey`, unit-tested) groups items whose tasks **share a `parent_task`** — or whose task **is** another in-queue item's parent (the subtree root) — into one card, and leaves everything else **standalone** (no fake grouping: an item with no shared root is keyed by its own id and never merges). Each collapsed card shows the **root title**, the **worst** member severity, the **oldest** member age, and the **per-kind counts** ("1 failed · 2 blocked"); it is **expandable** to the full member rows, and **actions are never hidden permanently** — one click on the header reveals every member's existing controls (retry / reopen / diagnose / investigate / inspect), each still backed by an existing route. A page **Group** toggle (default **on** — turning a fanned-out failure into one thing to look at) drops to the flat per-kind sections for operators who want every row laid out. Grouping is computed **after** search + filter (the page searches, then filters, then groups the visible set), so a group only ever holds already-matched items and "a group matches if any member matches" holds by construction; a kind filter that strands a single member degrades that subtree to a standalone row honestly. **Tests:** pure `inbox.test.ts` pins the group key (parent edge vs. parent-in-queue vs. standalone), the subtree collapse + unrelated-standalone split, the title resolution (root title → `parent_title` → id fallback), the worst-severity / oldest-age / per-kind-count rollups, the urgency ordering, and the search-then-group composition; backend `inbox_items_carry_parent_task_edge_for_subtree_grouping` pins that a reparented blocked child surfaces its `parent_task` + resolved `parent_title` while a top-level task carries none; render `inbox-group-render.test.mjs` drives the real page (the Group toggle, default on) and the real `SubtreeGroupCard` (the collapsed summary header; the expanded member rows **with** their action buttons) plus the standalone row. Deeper cross-**project** / **global** grouping (a whole goal tree, or grouping by orchestration run/step once the projection carries an orchestration id per item) remains future — the v1 above groups on the `parent_task` subtree edge only.
- **Cross-Guild search (IMPLEMENTED).** Closing the second of this section's "still future" items. The page gains a **search box** above the filter bar — a purely **local, free-text** cut over the items already loaded (it fetches nothing and grants no authority; it only narrows what is shown). Each item is matched (case-insensitive) against a blob of its **identity + content**: the category-prefixed id and every related id (`task_id` / `run_id` / `approval_id` / `continuation_id`), its `kind` / `severity` / `failure_class`, its `title` / `summary`, and its **action button labels** — so a search for a run id, `auth_required`, "retry", or a word in the title all hit. Multi-term is **AND** (every whitespace-separated term must appear). The query lives in the **URL** (`/inbox?q=…`, mirroring the Briefs `?brief=` / Agents `?agent=` pattern) so the narrowed view is **shareable** and survives refresh/back-forward; a **Clear** button appears when a query is set. Search composes with the existing kind/overdue **filter** (search first across all kinds, then the filter narrows it) — the filter **chip counts reflect the searched set**, so each chip honestly reads "how many of this kind match your search". When a search and/or filter narrows a non-empty queue to nothing, the empty state **names the active query verbatim** (and the filter scope when one is active — "No items match 'auth' in Failed runs.") via the pure `inboxSearchEmptyMessage`, distinct from both the per-filter line and the global all-clear. The pure helpers (`normalizeInboxQuery`, `inboxItemSearchText`, `inboxItemMatchesQuery`, `searchInbox`, `inboxSearchEmptyMessage`, `inboxFilterLabel`) live in `inbox.ts` and are unit-tested.
- **Tests.** Backend projection — `inbox_route_projects_only_attention_items_and_is_selective` (a fresh store is empty; a freshly *created* task is **not** surfaced — the queue is signal, not a task dump; blocking it surfaces a `blocked_task` item with honest fields + reopen actions). Frontend pure — `apps/dashboard/test/inbox.test.ts` pins the action-spec totality, the post/nav/seed mode split, the nav-target resolution, the grouping order, the badge count, the severity tones, and the per-kind investigation-seed gating. Render — `apps/dashboard/test/inbox-render.test.mjs` mounts the real page under the declarative router (no mount throw, honest pre-data state) and asserts the committed bundle carries it; `apps/dashboard/test/inbox-approval-render.test.mjs` drives the real `InboxRow` with a seeded pending-approval item and asserts the **inline** action set per shape (a tool-invocation approval → Approve & run / Allow always / Deny; a generic approval → Approve + Deny with the "nothing runs here" caveat, no allow-always) and the honest **degrade to Open approval** when no record is embedded. Backend embed — `inbox_pending_approval_item_embeds_full_record_for_inline_decisions` pins that a pending approval surfaces with the full `approval` record (status/action/risk, no bound invocation for a generic approval) and the `open_approval` nav action alongside. Ageing — backend `inbox_stamp_parse_and_logical_age_are_honest` pins the logical-stamp parse (and its fail-to-`None` for an unparseable stamp), the wrap-adjusted tick age, and the end-to-end "carry provenance, fill age only when it parses" helper; `inbox_items_carry_logical_age_and_sort_oldest_first` proves each item carries a real `attention_since` + `age_ticks` and that the earlier-blocked task (larger age) leads within its severity. Frontend pure — `inbox.test.ts` adds the bucket thresholds (half-open bands; null/negative/NaN → *unknown*, never silently *fresh*), the bucket labels/tones, the "ticks" detail (never wall-clock units), the kind/overdue filters + counts, and the filter-specific empty lines. Render — `inbox-age-render.test.mjs` drives the real `InboxRow` and asserts the *Overdue*/*Stale*/*Fresh* badge per band, the honest **age unavailable** for an unanchored item, the "logical clock" tooltip (no "seconds ago"), and the page's filter chips. Search — frontend pure `inbox.test.ts` pins the query normalization (trim/lowercase, blank → match-all), the searchable-text coverage (ids, kind/severity/failure class, content, action labels), the case-insensitive multi-term **AND** predicate, `searchInbox` (empty query passes all, order-preserving; a term hits across fields; a miss empties), `inboxFilterLabel`, and `inboxSearchEmptyMessage` (names the query verbatim + the filter scope, falls back to the filter line with no query); render `inbox-search-render.test.mjs` drives the real page (the labeled search input + placeholder, no **Clear** without a query, and a `?q=` URL pre-filling the input + offering **Clear**) and the real `InboxRow` over `searchInbox` (only the matching row renders for a hit, both for an empty query, none for a miss). The browser smoke clicks through the new **Inbox** nav route.

**Still future (deliberately):** deeper **cross-project / global** grouping (a whole goal tree, or grouping by orchestration run/step once the projection carries an orchestration id per item) — the shipped v1 collapses a stalled subtree on the `parent_task` edge only. The kind/overdue **filters**, the free-text **search**, and **cross-item subtree grouping** are now shipped above. A real wall-clock ageing signal is out of scope by design while the kernel runs on a deterministic logical clock — the v1 above is honest about measuring kernel-event ticks, not real time.
