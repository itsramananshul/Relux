...
**Still future work:** **skills/tags** and **per-agent budget** are not yet modeled (`relux_core::Agent` has neither field), so they are deliberately absent rather than shown as unenforced UI; the role-preset bundles and the richer scoped org/work/capability/autonomy toggles from §9 above also remain future work. This slice covers identity, role, persona, adapter, enabled/paused/disabled status, and the explicit-permission view + grant/revoke only.
