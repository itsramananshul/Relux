...
**Still pending (not built here, deliberately):** drag-to-change-status, list↔board toggle, sub-issue nesting / workflow-checklist rendering, and per-subtree progress strips and live cost. These remain §6/§7 targets. (Inline approve/reject/allow-always *on the strip* is now built — see the inline-decision bullet above; the dedicated Approvals surface remains the detailed audit/grants/permissions home.)
