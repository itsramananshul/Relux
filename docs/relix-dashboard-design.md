...
**Still pending (deliberately):** drag-to-reorder / reparent a subtask in the UI (the kernel cycle guard already covers reparenting safety); auto-rolling child progress onto the parent's status. (Per-subtree **cost** rollup is now built — see §6.5.) These remain §6/§7 targets.
