...
**Tests.** Pure classifier — `apps/dashboard/test/recovery.test.ts` pins each `failure_class` case (adapter / auth / permission / transient-scheduled / transient-exhausted / invalid / output-validation / cancelled / unknown), the retry-eligibility gating (no Retry when not retryable), the resumable add-on, the blocked-task cases (assigned → reopen+reassign, unassigned → assign-first + honest missing-info, last-failed-run root-cause fold), the non-blocked → null rule, and `latestRunForTask`. Render — `apps/dashboard/test/work-recovery-render.test.mjs` drives the real `RecoveryCard` with real assessments and asserts the eyebrow/root-cause/recommendation, a wired button, a navigation link, the muted pointer for an unwired action, the `role="note"` missing-info line, and the reassign picker (with the current operative marked). Dashboard `tsc --noEmit` clean, `npm test` green (495 + the new cases), `npm run build` green.

**Still pending (deliberately):** the §3.3b cheap **diagnostic LLM pass** (a status-only recovery agent that reads the logs and writes a richer narrative root cause) — this v1 uses the kernel's deterministic classification, not a model; the **Investigate → chat companion pre-loaded with the diagnosis** choice (the chat surface isn't wired to seed from a run yet); and a true **Inbox** queue (§5) that collects escalation cards across the Guild — today the cards live on the Work detail panels next to the work, and the oversight strip's *Needs attention* list (§6.1) is the closest Inbox surface.
