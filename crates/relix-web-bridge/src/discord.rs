//! HTTP proxies for the discord channel surface.
//!
//! Two read-only endpoints — the bridge does not stand up a
//! discord client of its own; both handlers call the configured
//! discord peer and parse the pipe-/tab-delimited body the node
//! returns.
//!
//! - `GET /v1/discord/status` — bot online flag + identity + channel.
//! - `GET /v1/discord/messages/recent?limit=20` — last N inbound
//!   messages from the controller's bounded ring, newest-first.
//!
//! Mirrors `telegram.rs` but with string snowflake ids.
...
const DEFAULT_PEER: &str = "discord";
...
pub async fn status(
    ...
    let body = call_peer_string(&state, &peer, "discord.status", &[]).await?;
    let parsed = parse_status_body(&body).ok_or((
        StatusCode::BAD_GATEWAY,
        Json(ApiError {
            error: format!("discord.status returned an unparseable body: {body:?}"),
        }),
    ))?;
...
async fn call_peer_string(
    ...
    match resp.res {
        ...
        ResponseResult::StreamHandle(_) => Err((
            StatusCode::BAD_GATEWAY,
            Json(ApiError {
                error: "unexpected stream response from discord peer".into(),
            }),
        )),
    }
}
...
mod tests {
    ...
    fn parse_status_typical_online_body() {
        let body = "online=true|username=relixbot|user_id=999|channel_id=12345678901234567|messages_seen=3|last_message_at=1700000000\n";
        let p = parse_status_body(body).unwrap();
        assert!(p.online);
        assert_eq!(p.username, "relixbot");
        assert_eq!(p.user_id, "999");
        assert_eq!(p.channel_id, "12345678901234567");
    ...
    fn parse_status_offline_body_with_sentinel_timestamp() {
        let body = "online=false|username=|user_id=|channel_id=12345678901234567|messages_seen=0|last_message_at=-1\n";
        let p = parse_status_body(body).unwrap();
        assert!(!p.online);
    ...
    fn parse_recent_typical_two_row_body() {
        ...
        assert_eq!(v[0].user_id, "2");
        assert_eq!(v[0].username, "bob");
        assert_eq!(v[0].channel_id, "20");
