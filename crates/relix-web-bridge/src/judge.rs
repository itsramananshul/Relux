//! RELIX-7.29 PART 4 — HTTP proxies for the `judge.*` caps.
//!
//! - `GET /v1/judge/verdicts?limit=20` → `judge.recent_verdicts`
//! - `GET /v1/judge/stats`             → `judge.stats`
...
pub async fn verdicts(
    ...
    body.insert("limit".into(), Value::from(q.limit.unwrap_or(20) as u64));
    match call_peer_json(&state, &peer, "judge.recent_verdicts", &Value::Object(body)).await {
...
pub async fn stats(
    ...
    use axum::response::IntoResponse;
    let peer = q.peer.clone().unwrap_or_else(|| DEFAULT_PEER.to_string());
    match call_peer_json(&state, &peer, "judge.stats", &Value::Null).await {
        Ok(v) => (StatusCode::OK, Json(v)).into_response(),
        Err(resp) => resp,
    }
}
